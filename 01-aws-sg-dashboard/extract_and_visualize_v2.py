...
Collect all AWS profile security group data across multi-account environments
"""
...
VPC_COLORS = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A', '#98D8C8', '#F7DC6F', '#BB8FCE', '#85C1E2']
...
# Additional fields per resource type (copied to nodes in generic loop)
                ...
                if source_vulns:
                    ...
                    for src in sources:
                        exposed_sgs[sg_id].append({
                            'exposed_via': source_sg, 'source': src
                        })
    return exposed_sgs
    ...
    with open(credentials_path, 'r') as f:
        for line in f:
            ...
            if match:
                ...
                # 'default' profile is used without a name
...
def reload_steampipe_service():
    ...
    if result.returncode != 0:
        if 'unknown state' in (result.stderr or '') or 'unknown state' in (result.stdout or ''):
            logger.warning("Steampipe unknown state detected, terminating via PID file...")
    ...
    if not _wait_for_steampipe_ready():
        logger.info("   ⚠️  Steampipe service may not be fully ready")
...
def check_aws_cli_auth(profiles, region=None):
    ...
    logger.info(f"\n   🔍 Checking AWS CLI authentication...")
    auth_ok = []
    for profile in profiles:
        try:
            result = subprocess.run(
            ...
            logger.error(f"{profile}: authentication failed - {e.stderr.strip()}")
        except json.JSONDecodeError:
            logger.error(f"{profile}: response parsing failed")
    return auth_ok

def get_all_account_identities(auth_ok_profiles):
    """Fetch account metadata via aggregator. Returns: {account_id: {profile_name, account_id, account_name}}"""
    logger.info(f"\n   🔍 Fetching account info via Steampipe aggregator...")
    try:
        account_query = """
        ...
        if missing:
            logger.warning(f"Missing profiles: {', '.join(missing)}")
...
def get_vpc_info():
    """Fetch VPC info via aggregator (all accounts in parallel). Returns: {account_id: {vpc_id: {name, color}}}"""
    ...
    # Single query: all VPC names (N+1 elimination) + account_id
    ...
    # Assign VPC colors per account (independent per-account, maintains per-profile behavior)
...
def get_rds_instances(all_vpc_info):
    """Fetch RDS instance info via aggregator (ID prefix prevents collisions)"""
    query = """
    SELECT
...
def get_load_balancers(all_vpc_info):
    ...
    for cfg in lb_configs:
        try:
            ...
            logger.warning(f"{cfg['type']} query error: {e}")
...
def get_vpc_endpoints(all_vpc_info):
    ...
    try:
        ...
        logger.warning(f"VPC Endpoint query error: {e}")
...
def get_lambda_functions(all_vpc_info):
    ...
    try:
        ...
        logger.warning(f"Lambda function query error: {e}")

    return lambda_info, lambda_sg_map

def _run_query_with_connection_fallback(query, auth_ok_profiles):
    """Execute aggregator query. Fall back to per-connection on AccessDenied."""
    try:
        ...
        if 'AccessDenied' not in error_text and 'not authorized' not in error_text:
            raise
        logger.warning("Aggregator access denied, retrying per-connection...")
        all_rows = []
        ...
        try:
            ...
            logger.warning(f"Skipping connection {conn_name}: {conn_err}")
        return {'rows': all_rows}

def _fetch_elasticache_shared_data(auth_ok_profiles):
    """Fetch ElastiCache shared data once (aggregator, AccessDenied fallback)"""
    cluster_query = """
...
def get_elasticache_clusters(all_vpc_info, clusters_result, subnet_groups_result):
    """Fetch ElastiCache cluster info (aggregator, ID prefix prevents collisions)"""
    elasticache_info = {}
    elasticache_sg_map = {}

    try:
        # subnet_to_vpc: (account_id, subnet_group_name) → vpc_id (prevents cross-account collisions)
        ...
        logger.warning(f"ElastiCache cluster query error: {e}")
...
def get_elasticache_replication_groups(all_vpc_info, clusters_result, subnet_groups_result, rg_result):
    """Fetch ElastiCache replication group info (aggregator, ID prefix prevents collisions)"""
    ...
    try:
        ...
        # (account_id, cluster_id) → subnet_group mapping
        ...
        # (account_id, subnet_group_name) → vpc_id mapping
        subnet_to_vpc = {(sg['account_id'], sg['cache_subnet_group_name']): sg['vpc_id']
                    for sg in subnet_groups_result.get('rows', [])
                    if sg.get('cache_subnet_group_name') and sg.get('vpc_id')}
        ...
        logger.warning(f"ElastiCache replication group query error: {e}")
...
def get_network_interfaces(all_vpc_info, known_ec2_ids=None):
    ...
    try:
        ...
        logger.warning(f"Network interface query error: {e}")

    return eni_info, eni_sg_map

def _resolve_vpc_from_sg(sg_id_list, sg_info):
    """Reverse lookup SG vpc_id to determine resource vpc_id"""
...
def get_ecs_services(all_vpc_info, sg_info):
    ...
    try:
        ...
        logger.warning(f"ECS service query error (skipping): {e}")
...
def get_redshift_clusters(all_vpc_info):
    ...
    try:
        ...
        logger.warning(f"Redshift cluster query error (skipping): {e}")
...
def get_opensearch_domains(all_vpc_info):
    ...
    try:
        ...
        logger.warning(f"OpenSearch domain query error (skipping): {e}")
...
def get_docdb_clusters(all_vpc_info, sg_info):
    ...
    try:
        ...
        logger.warning(f"DocumentDB query error (skipping): {e}")
...
def get_neptune_clusters(all_vpc_info, sg_info):
    ...
    try:
        ...
        logger.warning(f"Neptune query error (skipping): {e}")
...
def get_memorydb_clusters(all_vpc_info, sg_info):
    ...
    try:
        ...
        logger.warning(f"MemoryDB query error (skipping): {e}")
...
def get_security_groups_and_rules(all_vpc_info):
    """Fetch Security Group info and rules in single query via aggregator"""
    query = """
...
def generate_nodes_and_edges(resources, sg_info, sg_rules, extra_used_sgs=None):
    """Generate node and edge data (refactored: generic loop based on ResourceData)"""
    all_nodes = []
    all_edges = []
    ...
    for rd in resources:
        for res_id, info in rd.info.items():
            ...
            # Truncate long Lambda names for display
        if rd.type == 'lambda':
            label = label[:30]
    ...
    # SG → SG reference edges and usage tracking (CRITICAL FIX)
    for sg_id, rules in sg_rules.items():
        # Ingress rules
    ...
    # Update is_unused flag after SG-to-SG reference tracking
...
def _partition_by_account(info_dict, sg_map_dict):
    """Partition (info_dict, sg_map) by account_id. Restore prefixed key (account_id||res_id) to original res_id. Uses '||' delimiter to avoid mangling ARN-based IDs that contain ':'."""
    by_account = {}
...
def collect_multi_account_data(args=None):
    ...
    if not profiles:
        logger.info("⚠️  No AWS profiles found. Check ~/.aws/credentials")
        sys.exit(1)
    ...
    if not auth_ok_profiles:
        logger.warning(f"\n⚠️  All profiles failed authentication")
        sys.exit(1)

    if len(auth_ok_profiles) < len(profiles):
        failed = [p for p in profiles if p not in auth_ok_profiles]
        logger.warning(f"Excluding failed profiles: {', '.join(failed)}")

    if not skip_config:
        logger.info(f"\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        logger.info("🔧 Configuring Steampipe Aggregator...")
        logger.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    ...
    # Step 3: Fetch account metadata in batch
    accounts = get_all_account_identities(auth_ok_profiles)
    if not accounts:
        logger.error(f"\n⚠️  Failed to collect data from all profiles")
        sys.exit(1)
    ...
    # Step 4: VPC info (aggregator queries 3 accounts in parallel)
    logger.info(f"\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    logger.info("📊 Collecting all account resources via aggregator...")
    logger.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    all_vpc_info = get_vpc_info()

    # Step 5: Execute independent queries in parallel (ThreadPoolExecutor)
    logger.info("  ⏱  Starting parallel queries...")
    ...
    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
        ...
        try:
            ...
            logger.info(f"  ✓ {label}: {info_count} items")
        except Exception as e:
            logger.warning(f"{label} query failed (skipping): {e}")
            results[label] = ({}, {})

    parallel_elapsed = time.time() - parallel_start
    logger.info(f"  ⏱  Parallel queries completed: {parallel_elapsed:.1f}s")

    # SG data validation gate
    sg_info, sg_rules = results.get('SG', ({}, {}))
    if not sg_info:
        logger.info("❌ FATAL: Security Group query failed or no results. Aborting.")
        sys.exit(1)
    logger.info(f"  ✓ Security Group: {len(sg_info)} items")
    ...
    # Wave 2: sg_info-independent queries + ElastiCache post-processing in parallel
    ...
    def _safe_call(fn, label, *args):
        try:
            ...
            logger.warning(f"{label} query error (skipping): {e}")
        return {}, {}

    def _elasticache_cluster_wrapper():
        return get_elasticache_clusters(all_vpc_info, clusters_raw, subnet_groups_raw)

    def _elasticache_rg_wrapper():
        rg_raw = _run_query_with_connection_fallback(RG_QUERY, auth_ok_profiles)
        return get_elasticache_replication_groups(all_vpc_info, clusters_raw, subnet_groups_raw, rg_raw)

    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
        wave2 = {
        executor.submit(_safe_call, _elasticache_cluster_wrapper, 'ElastiCache Cluster'): 'ElastiCache Cluster',
        executor.submit(_safe_call, _elasticache_rg_wrapper, 'ElastiCache RG'): 'ElastiCache RG',
        executor.submit(_safe_call, get_redshift_clusters, 'Redshift', all_vpc_info): 'Redshift',
        executor.submit(_safe_call, get_opensearch_domains, 'OpenSearch', all_vpc_info): 'OpenSearch',
        executor.submit(_safe_call, get_eks_clusters, 'EKS', all_vpc_info): 'EKS',
        executor.submit(_safe_call, get_efs_mount_targets, 'EFS', all_vpc_info): 'EFS',
        executor.submit(_safe_call, get_network_interfaces, 'ENI', all_vpc_info, known_ec2_ids): 'ENI',
        }
        wave2_results = {}
        for future in as_completed(wave2, timeout=120):
            label = wave2[future]
            wave2_results[label] = future.result()
            info_count = len(wave2_results[label][0]) if isinstance(wave2_results[label], tuple) else 0
            logger.info(f"  ✓ {label}: {info_count} items")

    elasticache_info, elasticache_sg_map = wave2_results.get('ElastiCache Cluster', ({}, {}))
    elasticache_groups, elasticache_group_sg_map = wave2_results.get('ElastiCache RG', ({}, {}))
    redshift_info, redshift_sg_map = wave2_results.get('Redshift', ({}, {}))
    opensearch_info, opensearch_sg_map = wave2_results.get('OpenSearch', ({}, {}))
    eks_info, eks_sg_map = wave2_results.get('EKS', ({}, {}))
    efs_info, efs_sg_map = wave2_results.get('EFS', ({}, {}))
    eni_info, eni_sg_map = wave2_results.get('ENI', ({}, {}))

    # Wave 3: sg_info-dependent queries in parallel
    ecs_info, ecs_sg_map = {}, {}
    ...
    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
        wave3 = {
        executor.submit(_safe_call, get_ecs_services, 'ECS', all_vpc_info, sg_info): 'ECS',
        executor.submit(_safe_call, get_docdb_clusters, 'DocumentDB', all_vpc_info, sg_info): 'DocumentDB',
        executor.submit(_safe_call, get_neptune_clusters, 'Neptune', all_vpc_info, sg_info): 'Neptune',
        executor.submit(_safe_call, get_memorydb_clusters, 'MemoryDB', all_vpc_info, sg_info): 'MemoryDB',
        }
        for future in as_completed(wave3, timeout=120):
            label = wave3[future]
            result = future.result()
            info_count = len(result[0]) if isinstance(result, tuple) else 0
            logger.info(f"  ✓ {label}: {info_count} items")
        if label == 'ECS':
            ecs_info, ecs_sg_map = result
        elif label == 'DocumentDB':
            docdb_info, docdb_sg_map = result
        elif label == 'Neptune':
            neptune_info, neptune_sg_map = result
        elif label == 'MemoryDB':
            memorydb_info, memorydb_sg_map = result

    # Step 6: Partition by account_id
    ...
    # Step 7: Generate nodes/edges per account + enrichment
    all_data = {
        'nodes': [],
        'edges': [],
        'sg_rules': {},
    ...
    for account_id, account_meta in accounts.items():
        ...
        # Extract partitioned data for this account (empty dict if missing)
        ...
        # ENI safety net: sg_map needed even if eni_info is missing
        ...
        # ENI: no node creation needed, only track used_sgs
        eni_used_sgs = set()
